"""SlyTrade v0.9.9 CLI — ICT/SMC scalping bot (Layers 0-5 + live scalper)."""
...
app = typer.Typer(help="SlyTrade ICT/SMC scalper v0.9.9")
console = Console()
...
def doctor():
    ...
    table = Table(title="SlyTrade Doctor v0.9.9"); table.add_column("Check"); table.add_column("Status")
...
def live_cmd(
    ...
    """Run the live trading loop. Default: v0.9.9 champion persona (long-only A+/A/B RETEST_OB).
    ...
    persona_label = "SCALPER-UNRESTRICTED (long+short, all 4 setups)" if unrestricted else "v0.9.9 champion (long-only A+/A/B RETEST_OB)"
    console.print(f"[bold]SlyTrade LIVE v0.9.9[/bold] symbol={raw_symbol} live={live} "
